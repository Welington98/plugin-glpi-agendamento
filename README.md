...
- **Aba "Agendamentos" no chamado** — lista todos os agendamentos vinculados ao ticket (status, técnico, histórico) com ações rápidas de confirmar/concluir/cancelar sem sair do chamado
...
1. Faça o download ou clone este repositório dentro da pasta `plugins/` da sua instalação do GLPI, usando `agendamento` como nome do diretório:
   ```bash
   git clone https://github.com/Welington98/plugin-glpi-agendamento.git /var/www/glpi/plugins/agendamento
   ```

2. Acesse o GLPI como administrador e vá em:
...
Este repositório segue a convenção dos plugins oficiais do GLPI (como o [plugin-example](https://github.com/pluginsGLPI/example)): a raiz do repositório **é** o próprio plugin, então o `docker-compose.yml` monta a raiz do projeto diretamente em `plugins/agendamento` dentro do container do GLPI.
...
4. Acesse o GLPI em: [http://localhost:81](http://localhost:81)

> O serviço `glpi` aguarda o banco de dados estar saudável antes de iniciar (`depends_on` com `healthcheck`). No primeiro boot, a imagem `glpi/glpi` já instala o GLPI automaticamente (usuário/senha padrão: `glpi`/`glpi`); depois é só ativar o plugin em **Configuração → Plugins**.

### Serviços

| Serviço | Imagem           | Porta (host) |
|---------|------------------|--------------|
| glpi    | glpi/glpi:11.0.4 | 81           |
| db      | mysql:8.0        | (interna)    |

### Volumes

| Volume host          | Destino no container                  | Conteúdo                          |
|----------------------|----------------------------------------|------------------------------------|
| `.` (raiz do repo)   | `/var/www/glpi/plugins/agendamento`    | Código do plugin (este repositório)|
| `./storage/glpi`     | `/var/glpi`                            | Dados/config da instalação GLPI    |
| `./storage/mysql`    | `/var/lib/mysql`                       | Dados do banco MySQL               |
...
A raiz deste repositório é o próprio plugin (mesma convenção do [plugin-example](https://github.com/pluginsGLPI/example) do GLPI):
...
├── composer.json                    # Autoload PSR-4 e dependências de teste
├── phpunit.xml
├── front/
...
│   ├── ticket_agendamento.form.php  # Formulário de agendamento/edição/status a partir de um ticket
...
│       ├── agendamento-ticket.js            # Modal de criação na tela do ticket (timeline)
│       └── agendamento-ticket-tab.js        # Interação da aba "Agendamentos" no ticket
├── tests/
│   ├── bootstrap.php
│   └── Unit/AgendamentoHelpersTest.php
├── docker-compose.yml                # Ambiente de desenvolvimento (monta a raiz como plugin)
├── Dockerfile
└── storage/                          # Dados dos containers (gitignored), não faz parte do plugin
    ├── glpi/
    └── mysql/
```
...
|------------------------------|----------------------------------------------|
| `plugin_agendamento` → READ  | Visualização do calendário, aba do chamado e agendamentos |
...
O projeto utiliza **Semantic Release** com Conventional Commits. O CHANGELOG é gerado automaticamente em `CHANGELOG.md`.
